<template>
  <div class="data-mock">
    <a-page-header :title="tableName" subtitle="数据模拟" class="page-header" :show-back="true" @back="$router.back()">
      <template #extra>
        <div class="operate">
        ...
        <a-tooltip content="运行延迟">
          <a-input-number v-model="tableAttr.delay" :disabled="tableAttr.isBatch" style="width: 130px;margin: 0 5px"
            placeholder="运行延迟" mode="button" size="large" class="input-demo" />
        </a-tooltip>
        <a-tooltip content="运行次数">
          <a-input-number v-model="tableAttr.runCount" style="width: 130px; margin: 0 5px" placeholder="运行次数"
            mode="button" size="large" class="input-demo" />
        </a-tooltip>
        <a-button-group>
          <a-button size="large" status="danger" @click="editVisible = true">脚本<template #icon>
              <icon-code />
            </template></a-button>
          <a-button size="large"  status="warning"   @click="runMock(true)">测试<template #icon>
              <icon-record />
            </template>
          </a-button>
          <a-button size="large" status="success" @click="runMock(false)">运行
            <template #icon>
              <icon-play-arrow-fill />
            </template>
          </a-button>
          <a-button size="large" type="primary" @click="saveProTab">保存
            <template #icon>
              <icon-save />
            </template>
          </a-button>
    ...
    <a-table class="data-table" :columns="columns" :data="tableAttr.content" :loading="loading" :pagination="false"
      :scroll="scrollPercent" :scrollbar="true" column-resizable>
      <template #Remove="{ record }">
        <a-button class="oper_btn" type="text" size="large" shape="circle" :disabled="getIsRemove(record)"
      ...
      <template #Mock="{ record }">
          <codemirror v-model="record.Mock"   placeholder="支持Mock语法和{{}}表达式"
       :tab-size="2" />
      </template>
