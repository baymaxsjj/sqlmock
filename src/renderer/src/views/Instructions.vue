<template>
    <div class="instructions">
        <a-typography-title :heading="3">
            ...
            <a-button  type="text"  @click="toLink('https://gitee.com/baymaxsjj/sqlmock')" style="margin:0 15px">
                <template #icon>
        ...
        <a-typography-paragraph>
            支持Mock.js 语法<a-button type="text" @click="toLink('http://mockjs.com/examples.html')">访问-Mock.js官网</a-button>
            、支持Js脚本（通过<code v-pre>{{ 表达式 / 函数调用 }}</code>）、支持部分字段填充、支持表文档导出。
        </a-typography-paragraph>
        <a-typography-paragraph>
        ...
        <a-typography-title :heading="4">编写脚本（可选），<code>mockCount</code>运行次数、<code>Mock</code>mock.js
        </a-typography-title>
        <a-typography-paragraph>
            <a-image :src="gatRootFilePath('/image/doc/image-20221127195420902.png')" />
        </a-typography-paragraph>

        <a-typography-title :heading="4">编写表达式<code v-pre>@mock</code>、<code v-pre>{{ 表达式 }}</code>、<code v-pre>{{ Mock.xxx }}</code></a-typography-title>
        <a-typography-paragraph>
            注意<code v-pre>@mock</code>和<code v-pre>{{}}</code>一起使用时之间加个空格，要不然<code v-pre>@mock</code>无法使用</a-typography-paragraph>
        ...
        <a-typography-paragraph>
            如果批量插入有问题，请关闭批量
            单次插入次数过多有连接问题</a-typography-paragraph>
        ...
        <a-typography-paragraph>
            选择需要导出的表</a-typography-paragraph>
        ...
        <a-typography-title :heading="3">版权说明:未经本人允许禁止用于任何商业用途。仅限个人学习使用</a-typography-title>
    </div>
</template>
